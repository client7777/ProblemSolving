...
public class boj_15644
{
    ...
    static int bfs(int redX, int redY, int blueX, int blueY)
    {
        ...
        q.add(new Object[]{redX, redY, blueX, blueY, 1, ""});
        visit[redX][redY][blueX][blueY] = true;
        while (!q.isEmpty())
        {
            Object[] cur = q.poll();
            int curRedx  = (int)cur[0];
            int curRedy  = (int)cur[1];
            int curBluex = (int)cur[2];
            int curBluey = (int)cur[3];
            int curCnt   = (int)cur[4];
            String path = (String)cur[5];
            //기울인 횟수가 10회를 초과한 경우
            if(curCnt > 10)
            {
                return -1;
            ...
            for(int dir=0; dir<4; dir++)
            {
                ...
                {
                    //파란공은 구멍에 빠지지 않고 빨간 공만 구멍에 빠진 경우 성공이므로
                    //구슬이 구멍에 빠지는 시점의 방향을 추가해줘야 마지막 방향을 누락하지 않는다.
                    sb.append(path + getDirection(dir));
                    return curCnt; // 현재까지 기울인 횟수를 반환하고 함수 종료
                }
                ...
                if (!visit[nextRedx][nextRedy][nextBluex][nextBluey]) {
                    ...
                    q.add(new Object[]{nextRedx, nextRedy, nextBluex, nextBluey, curCnt + 1, path + getDirection(dir)});
